...
/** @brief Parse the Pd arguments for SDT Pd objects creation
This macro works for initializer functions whose signature is "classname_new(t_symbol *s, int argc, t_atom *argv)".
This macro defines an array of indices argi that stores the result of sdt_pd_arg_parse.
This macro makes the function return NULL if there are unused arguments (and also prints an error message)
...
/** @brief Get the Pd argument at the specified index. Arguments should have been parsed with SDT_PD_ARG_PARSE
...
@param[in] ... Append these arguments after the call to G (e.g. use ->field to access a field of its return value) */
#define GET_ARG(I, G, D, ...) (argi[ I ] >= 0)? G (argv + argi[ I ]) __VA_ARGS__ : D

/** @brief Get the Pd argument at the specified index as a window size (power of two). Arguments should have been parsed with SDT_PD_ARG_PARSE
@param[in] T The type of the variable
@param[in] X The name of the variable
@param[in] I The index of the argument
@param[in] D Default value if argument is not specified */
#define GET_ARG_WINSIZE(T, X, I, D) T _tmp_ ## X = GET_ARG(I, atom_getfloat, D); \
T X = SDT_nextPow2(_tmp_ ## X); \
if (_tmp_ ## X != X) \
  post("sdt.%s: Window size must be a power of 2, setting it to %d", s->s_name, X);
...
@param[in] T The SDT type (without the leading "SDT")
@param[in] F The name of the Pd object field where the SDT object is stored */
#define SDT_PD_FREE(T, F) if (x->key) \
  SDT_unregister ## T(x->key); \
SDT ## T ## _free(x-> F);
