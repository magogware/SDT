...
To use this macros you should define
 - The macro SDT_<TYPE> as the SDT type, without the leading "SDT". E.g. if the type is SDTExample, then
   \code{.c}#define SDT_EXAMPLE Example \endcode
 - The macro SDT_<TYPE>_NEW_ARGS as the arguments to pass on to the initializer function when instantiating during deserialization. E.g.
   \code{.c}#define SDT_EXAMPLE_NEW_ARGS 1024, 8 // -> Calls SDTExample_new(1024, 8) \endcode
 - The macro SDT_<TYPE>_ATTRIBUTES(T, A) to be the list of attributes. An attribute is defined inside the macro as "A(T, field, type, Field, key, json_type, default)", where
   - "field" is the field name of the attribute in the struct
   - "type" is the C type of the attribute in the struct
   - "Field" is the field name of the attribute as in the setter and getter functions (e.g. if the setter is "SDTType_setField", the "Field" is the field name)
   - "key" is the key name of the attribute as in the json file
   - "json_type" is the json type of the attribute as in json_parser (without the leading "json_")
...
A(T, p, double, Pi, pi, double, 3.14)\endcode Here it has been declared that
  - `SDTExample` has a field `x` of type `long`
   ...
   - the the setter function is: `void SDTExample_setAttributeX(SDTExample *x, long f)`
  ...
  - `SDTExample` has a field `p` of type `double`
   ...
   - the the setter function is: `void SDTExample_setPi(SDTExample *x, double f)`
...
#define SDT_TYPE_COPY_A(SDT_TYPE, F, T, G, K, J, D) CONCAT(SDT_TYPE_FULL(SDT_TYPE), CONCAT(_set, G))(dest, CONCAT(SDT_TYPE_FULL(SDT_TYPE), CONCAT(_get, G))(src));
#define SDT_TYPE_COPY(SDT_TYPE) SDT_TYPE_FULL(SDT_TYPE) *CONCAT(SDT_TYPE_FULL(SDT_TYPE), _copy)(SDT_TYPE_FULL(SDT_TYPE) *dest, const SDT_TYPE_FULL(SDT_TYPE) *src) { \
  SDT_TYPE ## _ATTRIBUTES(SDT_TYPE, SDT_TYPE_COPY_A) \
  return dest;                                                                                                                                                                     \
}
...
#define SDT_TYPE_GETTER_H(SDT_TYPE, F, T, G, K, J, D) extern T CONCAT(SDT_TYPE_FULL(SDT_TYPE), CONCAT(_get, G))(const SDT_TYPE_FULL(SDT_TYPE) *x);
#define SDT_TYPE_GETTER(SDT_TYPE, F, T, G, K, J, D) T CONCAT(SDT_TYPE_FULL(SDT_TYPE), CONCAT(_get, G))(const SDT_TYPE_FULL(SDT_TYPE) *x) { return x-> F; }
#define SDT_TYPE_SETTER_H(SDT_TYPE, F, T, S, K, J, D) extern void CONCAT(SDT_TYPE_FULL(SDT_TYPE), CONCAT(_set, S))(const SDT_TYPE_FULL(SDT_TYPE) *x, T f);
#define SDT_TYPE_SETTER(SDT_TYPE, F, T, S, K, J, D) void CONCAT(SDT_TYPE_FULL(SDT_TYPE), CONCAT(_set, S))(const SDT_TYPE_FULL(SDT_TYPE) *x, T f) { x-> F = f; }

/** @brief Create getter function declarations for all attributes \par
See the example at the beginning of the section for an explanation of the generated functions
@param[in] SDT_TYPE The SDT type macro */
#define SDT_TYPE_MAKE_GETTERS_H(SDT_TYPE) SDT_TYPE ## _ATTRIBUTES(SDT_TYPE, SDT_TYPE_GETTER_H)

/** @brief Create getter functions for all attributes \par
See the example at the beginning of the section for an explanation of the generated functions
@param[in] SDT_TYPE The SDT type macro */
#define SDT_TYPE_MAKE_GETTERS(SDT_TYPE) SDT_TYPE ## _ATTRIBUTES(SDT_TYPE, SDT_TYPE_GETTER)

/** @brief Create setter function declarations for all attributes \par
See the example at the beginning of the section for an explanation of the generated functions
@param[in] SDT_TYPE The SDT type macro */
#define SDT_TYPE_MAKE_SETTERS_H(SDT_TYPE) SDT_TYPE ## _ATTRIBUTES(SDT_TYPE, SDT_TYPE_SETTER_H)

/** @brief Create setter functions for all attributes \par
See the example at the beginning of the section for an explanation of the generated functions
@param[in] SDT_TYPE The SDT type macro */
#define SDT_TYPE_MAKE_SETTERS(SDT_TYPE) SDT_TYPE ## _ATTRIBUTES(SDT_TYPE, SDT_TYPE_SETTER)
...
@param[in] SDT_TYPE The SDT type macro */
#define SDT_JSON_SERIALIZE_H(SDT_TYPE) extern json_value * CONCAT(SDT_TYPE_FULL(SDT_TYPE), _toJSON) (const SDT_TYPE_FULL(SDT_TYPE) *x);

/** @brief Create the serialization function \par
Following the example, the function would have the signature \code{.c}json_value *SDTExample_toJSON(const SDTExample *x);\endcode
The newly created `json_value` instance will have to be manually deallocated
@param[in] SDT_TYPE The SDT type macro */
#define SDT_JSON_SERIALIZE(SDT_TYPE) json_value * CONCAT(SDT_TYPE_FULL(SDT_TYPE), _toJSON) (const SDT_TYPE_FULL(SDT_TYPE) *x) { \
  json_value *obj = json_object_new(0); \
  SDT_TYPE ## _ATTRIBUTES(SDT_TYPE, SDT_JSON_PUSH) \
  return obj; \
}

/** @brief Create the deserialization function declaration
@param[in] SDT_TYPE The SDT type macro */
#define SDT_JSON_DESERIALIZE_H(SDT_TYPE) extern SDT_TYPE_FULL(SDT_TYPE) *CONCAT(SDT_TYPE_FULL(SDT_TYPE), _fromJSON)(const json_value *x);

/** @brief Create the deserialization function \par
Following the example, the function would have the signature \code{.c}SDTExample *SDTExample_fromJSON(const json_value *x);\endcode
The newly created instance will have to be manually deallocated
@param[in] SDT_TYPE The SDT type macro */
...
@param[in] T Name of the SDT type (without the leading "SDT")
@param[in] X Name of the SDTHashmap global variable */
#define SDT_HASHMAP_GET(T, X) SDT ## T *SDT_get ## T(const char *key) { \
...
/** @brief Define SDTHashmap related function declarations (register, get, unregister)
@param[in] SDT_TYPE The SDT type macro */
#define SDT_DEFINE_HASHMAP_H(SDT_TYPE) SDT_HASHMAP_REGISTER_H(SDT_TYPE, hashmap_ ## SDT_TYPE) \
...
@param[in] SDT_TYPE The SDT type macro
@param[in] S Size of the SDTHashmap */
#define SDT_DEFINE_HASHMAP(SDT_TYPE, S) SDT_DEFINE_HASHMAP_GLOBAL(hashmap_ ## SDT_TYPE) \
